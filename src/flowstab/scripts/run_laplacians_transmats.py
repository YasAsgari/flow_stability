...
def worker(net_start_stop):
    ...
    if save_static_adjacencies:
        ...
        if not os.path.exists(adj_file):
            ...
            print(f"PID {os.getpid()}:",
                  f"computing and saving static adjacency matrix, took {t1-t0}")
    ...
    if (all_inter_expm_exists and all_inter_lin_exists and not save_slice_trans) or \
        ...
        try:
            ...
            t1 = time.time()
            print(f"PID {os.getpid()}:",f"finished in {t1 - t0:.2f}" )
            ...
            if compute_lin_transmat:
                ...
                t2 = time.time()
                print(f"PID {os.getpid()}:",f"finished in {t2 - t1:.2f}")
